<%@ Control Language="C#" AutoEventWireup="true" CodeFile="FAQpageBarControl.ascx.cs" Inherits="Control_FAQpageBarControl" %>
...
<%= DataSource.TotalRow %>条数据，共<%= DataSource.TotalPage %>页
	...
	<a href="<%= BaseURL + "p=" + i %>" class="pagebar-prev"><%= i%></a>
  
	<%}
    }%>
	  
	<a href="<%= BaseURL + "p=" + PageIndex %>" class="pagebar-current"><%= PageIndex%></a>

	<% for (int j = PageIndex + 1; j < PageIndex + 4 && j <= DataSource.TotalPage; j++)
    {%>
   
	<a href="<%= BaseURL + "p=" + j %>" class="pagebar-next"><%= j%></a>
